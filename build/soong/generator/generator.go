...
func init() {
	android.RegisterModuleType("franxx_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = franxxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = franxxExpandVariables(ctx, dep_file)
	...
	exCmd := franxxExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "franxx.sbox.textproto"
	desc := "generate"
	name := "generator"
